...
function solve(elves: Set<string>, rounds = 10): number {
  ...
  return getEmptyGroundTiles(currElves); 
}
...
function getEmptyGroundTiles(elves: Set<string>): number {
  const b = getBounds(Array.from(elves).map(deserializePoint));
  return (b.x.max - b.x.min + 1) * (b.y.max - b.y.min + 1) - elves.size;
}
