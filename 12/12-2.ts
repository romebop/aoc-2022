...
function solve(map: string[][]): number {
  const startMarker = getPoint(map, 'S')!;
  map[startMarker.y][startMarker.x] = 'a';
  const end = getPoint(map, 'E')!;
  map[end.y][end.x] = 'z';
  return getStartPoints(map)
...
function getStartPoints(map: string[][]): Point[] {
  const startPoints: Point[] = [];
  for (let y = 0; y < map.length; y++) {
    for (let x = 0; x < map[0].length; x++) {
      if (['S', 'a'].includes(map[y][x])) startPoints.push({ x, y });
    }
  }
  return startPoints;
}